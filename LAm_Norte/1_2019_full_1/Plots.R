...
prj <- reptoRlist("./salidas/1_2019_2_full.prj")
...
dir.create('../Figs/2019_catch')

# Rec_total
p1 <- ggplot(data = NULL, aes(x = c(yrs, lastyr+1, lastyr+2))) + 
  geom_line(aes(y = c(prj$Reclutas[,2],rep(NA,2)), colour = 'b_19', linetype = 'b_19')) +
...
ggsave(p1, filename = "../Figs/2019_catch/Rec_1921_catch.png", width=8.5, height=5.5, dpi=300)


# SSB
p2 <- ggplot(data = NULL, aes(x = c(yrs, lastyr+1, lastyr+2))) + 
  geom_line(aes(y = c(prj$SSB[,2],rep(NA,2)), colour = 'b_19', linetype = 'b_19')) +
...
p2
ggsave(p2, filename = "../Figs/2019_catch/SSB_1921_catch.png", width=8.5, height=5.5, dpi=300)


# Fh
p3 <- ggplot(data = NULL, aes(x = c(yrs, lastyr+1, lastyr+2))) + 
  geom_line(aes(y = c(prj$F_total[,2],rep(NA,2)), colour = 'b_19', linetype = 'b_19')) +
...
p3
ggsave(p3, filename = "../Figs/2019_catch/Fh_1921_catch.png", width=8.5, height=5.5, dpi=300)


# Catch
p4 <- ggplot(data = NULL, aes(x = c(yrs, lastyr+1, lastyr+2))) + 
  geom_line(aes(y = c(prj$Y_total[,2],rep(NA,2)), colour = 'b_19', linetype = 'b_19')) +
...
p4
ggsave(p4, filename = "../Figs/2019_catch/Catch_1921_catch.png", width=8.5, height=5.5, dpi=300)


# Read proyecciones opcion Rec 2
prj_2 <- reptoRlist("./salidas/Rec2.prj")
prj_3 <- reptoRlist("./salidas/Rec3.prj")
...
p5
ggsave(p5, filename = "../Figs/2019_catch/CatchRecs_1921_catch.png", width=8.5, height=5.5, dpi=300)
